function getUserByQR(qr) {
    ...
    $.post("./camera/", {_csrf: $("#_csrf").val(), imgdata: "qr"}, function(data) {
...
function getLogs() {
    ...
    $.post("./camera/logs", {_csrf: $("#_csrf").val()}, function(data) {
        var js = jQuery.parseJSON(data);
...
var intId = setInterval(function(){ //clearInterval(timerId);
    var isPong = false;
    $.post("http://localhost:8080/workShop/ping", {_csrf: $("#_csrf").val()}, function(data) {
        isPong = true;  
    });
...
$(document).ready(function() {
    ...
    $('#addGuestBtn').click(function() {
        ...
        $.get("http://localhost:8080/workShop/camera/guest", {}, function(data) {
    ...
    $("#w-personal-search").autocomplete({
        source: function(request, response) {
            ...
            $.getJSON("./camera/plist", {
                personalName: extractLast(request.term)
            }, response);
        },
        select: function(event, ui) {
            var newWin = window.open("./camera/profile/"+ui.item.value,
                    "JSSite",
